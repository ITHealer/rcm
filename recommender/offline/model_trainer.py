...
- POSITIVE_ACTIONS đã cập nhật: like, love, laugh, wow, sad, angry, care, comment, share, save
- Sample weight (time-decay) có thể nhân với action multipliers:
  * view: 0.5
...
class ModelTrainer:
    ...
    def __init__(self, config: dict):
        ...
        self.num_boost_round = int(model_cfg.get("num_boost_round", 200))
        ...
        self.half_life_days = int(tr_cfg.get("half_life_days", 7))
        self.min_weight = float(tr_cfg.get("min_weight", 1e-3))
        self.use_action_multiplier = bool(tr_cfg.get("use_action_multiplier", True))
        # default multipliers include new reactions
        self.action_multipliers = tr_cfg.get("action_multipliers", {
            "view": 0.5,
            "like": 1.0, "love": 1.3, "care": 1.25, "laugh": 1.2, "wow": 1.1, "sad": 0.9, "angry": 0.9,
            "comment": 1.5, "share": 2.0, "save": 1.2,
        })
    ...
    # ----------------------------------------------------------------------
    # 1) PREPARE TRAINING DATA (giữ evaluate flow cũ)
    # ----------------------------------------------------------------------
    def prepare_training_data(
        self,
        interactions_df: pd.DataFrame,
        ...
        df.rename(columns={"UserId": "user_id", "PostId": "post_id", "ReactionTypeId": "reaction_type_id", "CreateDate": "created_at"}, inplace=True)

        has_weights = "weight" in df.columns
        if has_weights:
            print("   ✅ Detected 'weight' column")
        else:
            print("   ℹ️  No 'weight' found. Will compute later via sample_weight in train().")

        # Split positives/negatives theo POSITIVE_ACTIONS mới
        ...
        if "weight" in training_df.columns:
            ...
            print(f"   Weights: mean={w.mean():.4f}, median={w.median():.4f}, "
    ...
    # ----------------------------------------------------------------------
    # 2) TRAIN (tự tính sample_weight theo time-decay + multipliers)
    # ----------------------------------------------------------------------
    def train(self, train_df: pd.DataFrame, val_df: pd.DataFrame) -> Tuple[lgb.Booster, StandardScaler, list]:
        print("\n🔧 Training Ranking Model...")

        # tự tính sample_weight nếu chưa có: w = multiplier(action not available here) * time_decay
        # Ở đây train_df/val_df chỉ có features/label(/weight). Time-decay đã tính ở bước build features trước,
        # hoặc bạn có thể tính ngoài rồi truyền vào 'weight'. Nếu không có, LightGBM sẽ train không weight.
        has_weights = "weight" in train_df.columns and "weight" in val_df.columns
        if has_weights:
        ...
        train_w = train_df["weight"].values if has_weights else None
        val_w = val_df["weight"].values if has_weights else None
        ...
        if has_weights:
            ...
            dval = lgb.Dataset(X_val_sc, label=y_val, reference=dtrain)
